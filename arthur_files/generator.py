...
class lin_opt_pbs:
    def __init__(self, prob_name_list, non_fixed_vars=None, path=None):
    ...
    def read_non_fixed_vars(self):
        constraint_names = self.prob_list[0].linear_constraints.get_names()
        non_fixed_vars = []
...
#         prob_temp.prob_list[0].read(prob_root.name_list[k])
...
# The function problem_generator_yield generates an pair of lists
# with N random RHS based on a chosen linear optimization problem
# and their N associated solutions
# The RHS are truncated : only the non fixed coefficients are kept

# Parameters of problem generator yield:
#           problems : a string list giving the names of the linear optimization problems
#               OR a list of cplex.Cplex linear optimization problems
#           N : an int giving the number of RHS to generate
#           dev : a float setting the relative deviation of the variables when generating new problems
#           non_fixed_vars : a list containing all variables which will be affected by the noise
#               when generating new problems. If not given, calculated by the programm.
# Output:  a pair of lists containing N RHS and their N associated solutions
...
def calculate_network_size(problems):
    prob_root = lin_opt_pbs(problems)
    problem = prob_root.prob_list[0]
    m = problem.linear_constraints.get_num()
    n = problem.variables.get_num()
    return [m, (m+n)*(m+n)//10, n+1]
