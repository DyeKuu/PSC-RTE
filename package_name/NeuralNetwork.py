...
class nn:
    def __init__(self):
        ...
        reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.7, verbose=1, min_delta=1e-5, patience=2, cooldown=2,
                                      min_lr=0.000001)
    ...
    def basic_nn(self, list_n_neurons, last_activation=None):
        ...
        assert last_activation == "relu" or last_activation == "sigmoid" or last_activation is None, "This activation name does not exist :D"
    ...
    def fit(self, pb_train, sol_train, epochs, validation_split, batch_size):
        return self.model.fit(x=pb_train, y=sol_train, epochs=epochs, validation_split=validation_split,
                              batch_size=batch_size, callbacks=self.callback)

    def add_processing_linear_mean(self):
        """trains with a linear transform for the solutions so that mean is 0.5 and the values are between 0 and 1.
        Choosing factor > 2 makes you put the values tighter around 0.5 """
    ...
    def add_processing_standard(self):
        self.pre_processing.append(["linear_standard", None, None, 0, 0])
        # include ("linear_standard", list_sigma_coordinate_RHS, list_mean_coordinate_RHS, sigma_sol, mean_sol)
    ...
    def predict(self, dataset_instance):
        ...
        object_to_analyze = to_analyze(new_dataset_instance.get_solutions(),
    ...
    def train_with(self, initial_dataset_instance, epochs, validation_split, batch_siz):
        ...
        history = self.fit(dataset_instance.get_RHS(), dataset_instance.get_solutions(), epochs=epochs,
                           validation_split=validation_split, batch_size=batch_siz)  # training the network
